...
const map_menu = new senjs.util.List(menu),
    REQUEST_NEAR_BY = 1,
...
export class MapPage extends senjs.layout.FrameLayout {
    ...
    constructor() {
        ...
        this._cache = {
            listPlaces: new senjs.util.List()
        }
    ...
    initControl() {
        this._view.map_view = new senjs.lib.GoogleMap("100%", "100%")
            .toFillParent();

        this._view.layout_menu = new senjs.layout.DrawerLayout("100%", "30%")
            .setDirection(senjs.constant.Direction.TOP);

        this._view.btn_show_list = new senjs.widget.FloatingButton("keyboard_arrow_up")
            .toBottomParent().toLeftParent().setLeft(10).setBottom(10)
            .setBackground(senjs.res.material_colors.Blue.g500)
            .setIconColor("#fff");

        this._view.drawer_places = new senjs.layout.DrawerLayout("100%", "100%").setDirection(senjs.constant.Direction.BOTTOM);

        this._view.line_ios_task = new senjs.layout.FrameLayout("50%", 4)
            .toBottomParent().setBottom(10).setRadius(5)
            .setLeft("25%")
            .setBackground("rgba(255,255,255,0.8)").setShadow("rgba(0,0,0,0.1)", 0, 0, 2);


        var pn_top_header = new senjs.layout.FrameLayout()
            .toTopParent().setLeft("30%").setRight("30%")
            .setRadiusAt(0, 0, 6, 6)
            .setBackground("#fff")
            .setShadow("rgba(0,0,0,0.3)", 0, 0, 4);

        var icon_dropdown = new senjs.widget.IconView("arrow_drop_down")
            .toRightParent().toTopParent().toBottomParent().setIconSize(senjs.res.dimen.icon.tiny);

        this._view.label_top = new senjs.widget.TextView();
        ...
        this._view.lsv_menu = new senjs.widget.ListView().toFillParent();
        this._view.lsv_place = new senjs.widget.ListView().toFillParent()
    ...
    initData() {
        this._adapter.adapter_menu = new senjs.adapter.BaseAdapter(map_menu);
        this._adapter.adapter_menu.setColumn(3);
        this._adapter.adapter_menu.setView((dataItem, position, convertView) => {
            return new senjs.widget.TextView()
                .setWidth("94%")
                .setLeft("3%")
                .setTop(5)
                .setTextGravity(senjs.constant.Gravity.CENTER)
        ...
        this._adapter.adapter_place = new senjs.adapter.BaseAdapter(new senjs.util.List());
        this._adapter.adapter_place.setView((dataItem, position, convertView) => {
            var lb_name, lb_address, imv_thum,
                line_name, line_address;
            if (convertView == null) {
                convertView = new senjs.layout.LinearLayout("92%")
                    .setTop(15)
                    .setOrientation(senjs.constant.Orientation.VERTICAL)
                    .setLeft("4%").setBackground("#fff").setRadius(4).setPadding(10);
                imv_thum = new senjs.widget.ImageView().setWidth("100%").setHeight(240);
                imv_thum.setBackground(senjs.res.material_colors.Grey.g300).setScaleType(senjs.constant.ImageScale.COVER)
                line_name = new senjs.layout.LinearLayout("100%").setTop(10);
                line_address = new senjs.layout.LinearLayout("100%").setTop(6);
                lb_name = new senjs.widget.TextView().setTextSize(senjs.res.dimen.font.larger).bold()
                    .setTextColor(senjs.res.material_colors.Grey.g700);
                lb_address = new senjs.widget.TextView().setTextSize(senjs.res.dimen.font.normal)
                    .setTextColor(senjs.res.material_colors.Grey.g700);

                line_name
                    .addView(new senjs.widget.IconView("store").setIconSize(senjs.res.dimen.icon.tiny).setRight(6))
                    .addView(lb_name)

                line_address
                    .addView(new senjs.widget.IconView("location_on").setIconSize(senjs.res.dimen.icon.tiny).setRight(6))
                    .addView(lb_address)
    ...
    onResponsePlaceSuccess(request_case, result) {
        switch (request_case) {
            case REQUEST_NEAR_BY:
                ...
                this._cache.listPlaces = new senjs.util.List(result);
...
class PlaceDetailPage extends senjs.layout.FrameLayout {
    ...
    initData() {
        this._adapter.adapter_photo = new senjs.adapter.BaseAdapter(new senjs.util.List(this._meta.placeDetail.photos))


        this._adapter.adapter_photo.setView((photo, position, convertView) => {
            return new senjs.widget.ImageView(photo.getUrl({
                maxWidth: 640,
                maxHeight: 640
            })).setScaleType(senjs.constant.ImageScale.COVER).toFillParent();
        });
    ...
    initControl() {
        ...
        var panel_banner = new senjs.layout.FrameLayout("100%", 240);

        this._view.pager_photo = new senjs.layout.PagerLayout().setWidth("100%").setHeight("100%").setBackground(senjs.res.material_colors.Grey.g300);

        this._view.linear_paging = new senjs.layout.LinearLayout("100%", 30);
        this._view.linear_paging.setOrientation(senjs.constant.Orientation.HORIZONTAL);

        this._view.lb_address = new senjs.widget.TextView().setTextSize(senjs.res.dimen.font.small).setWidth("100%");
        this._view.lb_phone = new senjs.widget.TextView().setTextSize(senjs.res.dimen.font.small).setWidth("100%");
        this._view.lb_email = new senjs.widget.TextView().setTextSize(senjs.res.dimen.font.small).setWidth("100%");
        this._view.lb_rating = new senjs.widget.TextView().setPadding(5)
            .bold()
            .setTextSize(senjs.res.dimen.font.x_extreme)
            .setTextGravity(senjs.constant.Gravity.CENTER);

        this._view.btn_review = new senjs.widget.Button((this._meta.placeDetail.reviews ? this._meta.placeDetail.reviews.length : 0) + " reviews")
            .setWidth("94%").setTop(15).setLeft("3%").setBackground(senjs.res.material_colors.Grey.g100)
            .setRadius(4).setPadding(10);

        var linear_info = new senjs.layout.LinearLayout().setWidth("78%").setTop(5).setLeft("2%");
        linear_info.setOrientation(senjs.constant.Orientation.VERTICAL);

        var line1 = new senjs.layout.LinearLayout().setWidth("100%");

        var line_address = new senjs.layout.LinearLayout().setPadding(5);
        var line_phone = new senjs.layout.LinearLayout().setPadding(5);
        var line_email = new senjs.layout.LinearLayout().setPadding(5);
        var line_rating = new senjs.layout.LinearLayout().setWidth("20%").setPadding(5).setTop(10).setBottom(10).setBorderLeft(1, senjs.res.material_colors.Grey.g200);

        var icon_address = new senjs.widget.IconView("location_on").setIconSize(senjs.res.dimen.icon.tiny).setRight(8);
        var icon_phone = new senjs.widget.IconView("phone").setIconSize(senjs.res.dimen.icon.tiny).setRight(8);
        var icon_email = new senjs.widget.IconView("link").setIconSize(senjs.res.dimen.icon.tiny).setRight(8);
        var icon_rating = new senjs.widget.IconView("star_rate")
            .setWidth("38%")
            .setIconSize(senjs.res.dimen.icon.normal).setRight(2);
        ...
        if (this._meta.placeDetail.photos && this._meta.placeDetail.photos.length > 0) {
            for (let i = 0; i < this._meta.placeDetail.photos.length; i++) {
                var ic_pager = new senjs.widget.IconView("fiber_manual_record")
                    .setIconSize(senjs.res.dimen.icon.x_tiny).setIconColor(i == 0 ? "rgba(255,255,255,0.8)" : "rgba(0,0,0,0.6)").setPadding(2);
        ...
        this._view.linear_paging.setGravity(senjs.constant.Gravity.TOP_CENTER);
    ...
    showReview() {
        ...
        reviewInstance.getPageOpening().setBackground(senjs.res.material_colors.Grey.g200);
        var linear_comment = new senjs.layout.LinearLayout("100%");
        linear_comment.setOrientation(senjs.constant.Orientation.VERTICAL);
        var render_comment_view = (dataItem, position) => {
            var convertView = new senjs.layout.LinearLayout("100%").setPadding(10);
            var pn_content = new senjs.layout.LinearLayout().setLeft(15);
            var imv_avatar, lb_author, lb_comment, lb_time;

            convertView.setBackground("transparent");
            imv_avatar = new senjs.widget.ImageView()
                .setWidth(48)
                .setHeight(48)
                .setRadius(24);

            pn_content.setWidth("calc(100% - 48px");
            pn_content.setOrientation(senjs.constant.Orientation.VERTICAL);

            lb_author = new senjs.widget.TextView()
                .setWidth("100%")
                .setTextSize(senjs.res.dimen.font.smaller);

            lb_comment = new senjs.widget.TextView().setWidth("100%")
                .setTop(10)
                .setBottom(10);
            lb_time = new senjs.widget.TextView()
                .setWidth("100%")
                .setTextSize(senjs.res.dimen.font.smaller);

            pn_content.setRadius(4).setBackground("#fff");
            lb_time.setTextAlign(senjs.constant.TextAlign.RIGHT);
        ...
        new senjs.util.List(this._meta.placeDetail.reviews).foreach((item, position) => {
